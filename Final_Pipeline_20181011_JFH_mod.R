...
plot_ordination(microbiome_subset, ordu3, color="prev_type", shape = "type", title=paste("WuniFrac NMDS")) + geom_point(size=5) + coord_fixed() + 
  scale_color_manual(values = c('darkgoldenrod2', 'turquoise4')) + 
  scale_shape_manual(values=c(19,1)) + 
  geom_text(mapping = aes(label = timepoint), size = 3.5, vjust = 2) +
  geom_line(aes(group = fish), linetype="dashed", color="gray")
...
adonis_res
adonis_res <- adonis2(formula = phydist ~ metaD$testosterone, data = metaD) 
adonis_res
...
faith <- pd(t(data.frame(otu_table(phyloseq_data_norarify))), ep_tree, include.root=FALSE)
...
# Comparison of Species Richness based on 100 iterations of rarefication and raw counts
...
alphadiv_for_subset <- cbind(SR_ave, SR_se, richness, Faith_norar = faith$PD, prev_type = meta_table2$prev_type, behavior_scores = meta_table2$behavior_scores, weight = meta_table2$weight, length = meta_table2$length, bc.res = meta_table2$bc.res, t_byLeng = meta_table2$t_byLeng, otu4_deseq = meta_table2$deseq_norm_OTU_4, otu6_deseq = meta_table2$deseq_norm_OTU_6, otu21_deseq = meta_table2$deseq_norm_OTU_21, otu143_deseq = meta_table2$deseq_norm_OTU_143, otu474_deseq = meta_table2$deseq_norm_OTU_474, otu438_deseq = meta_table2$deseq_norm_OTU_438, otu376_deseq = meta_table2$deseq_norm_OTU_376, otu155_deseq = meta_table2$deseq_norm_OTU_155, otu125_deseq = meta_table2$deseq_norm_OTU_125, otu4_rar = meta_table2$OTU_4, otu6_rar = meta_table2$OTU_6.1, otu21_rar = meta_table2$OTU_21, otu143_rar = meta_table2$OTU_143, otu474_rar = meta_table2$OTU_474, otu438_rar = meta_table2$OTU_438, otu376_rar = meta_table2$OTU_376, otu155_rar = meta_table2$OTU_155, otu125_rar = meta_table2$OTU_125)
...
###########################################
#alphadiv <- read.table(file = "richness+meta_for_Cynthia_20181212.csv", sep = ",", header = TRUE) # I added length to this
alphadiv <- cbind(alphadiv,length_mm = alphadiv$length/10)
dim(alphadiv)
...
alphadiv.t <- alphadiv[!is.na(alphadiv$t_corrected),]  # need to get rid of the two NAs, this column is raw T/ length mm
plot(alphadiv.t$t_corrected ~ alphadiv.t$length_mm, col = ifelse(alphadiv.t$type=="S","cyan", "gold"), pch = 19)
plot(alphadiv.t$t_corrected ~ alphadiv.t$type, col = c("darkgoldenrod2", "turquoise4")) # josh to ggplot this

t.lmer<-lmer(alphadiv.t$t_corrected~alphadiv.t$type+(1|alphadiv.t$fish));summary(t.lmer) # P = 0.0401, t = -2.158, obs= 32
...
alphadiv<-alphadiv%>%filter(Sample!="SS.AS12.1")
...
SR_ave_lm<-lm(data=alphadiv, SR^(-0.25)~prev_type*type);plot(resid(SR_ave_lm));shapiro.test(resid(SR_ave_lm));hist(resid(SR_ave_lm))
...
SR_ave_lm_global<-lm(data=alphadiv, SR^(-0.25)~prev_type*type)
...
#make bar plots for residual testosterone vs. current phenotype with colours
...
#make bar plots for uncorrected testosterone vs. current phenotype with colours
AVG<-aggregate(testosterone ~ type  , meta_table3, mean, na.rm = T) 
SE<-aggregate(testosterone ~ type  , meta_table3, ste) 
ptypetest <- data.frame(merge(AVG, SE, by = "type"))

p <- ggplot(ptypetest, aes(x=factor(type), y=testosterone.x, fill = type)) + 
  stat_summary(fun.y="mean", geom="bar") + 
  geom_errorbar(aes(ymin=ptypetest$testosterone.x - ptypetest$testosterone.y, ymax=ptypetest$testosterone.x + ptypetest$testosterone.y),
                width=0.2,                    # Width of the error bars
                position=position_dodge(.9)) + labs(x = "Current Phenotype", y = "Testosterone (pg/hr)") 
p + scale_fill_manual(values = c('darkgoldenrod2', 'turquoise4'))
...
#make bar plots for weight vs. current phenotype
AVG<-aggregate(weight ~ type  , meta_table3, mean, na.rm = T) 
SE<-aggregate(weight ~ type  , meta_table3, ste) 
wvstype <- data.frame(merge(AVG, SE, by = "type"))

x <- ggplot(wvstype, aes(x=factor(type), y=weight.x, fill = type)) + 
  stat_summary(fun.y="mean", geom="bar") + 
  geom_errorbar(aes(ymin=wvstype$weight.x - wvstype$weight.y, ymax=wvstype$weight.x + wvstype$weight.y),
                width=0.2,                    # Width of the error bars
                position=position_dodge(.9)) + labs(x = "Current Phenotype", y = "Weight")
x + scale_fill_manual(values = c('darkgoldenrod2', 'turquoise4'))
...
smry <- pf.summary(PF,pf_tax,factor=16)
pf.tidy(smry)
...
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=2)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=3)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=4)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=5)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=6)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=7)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=8)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=9)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=10)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=11)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=12)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=13)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=14)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=15)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=16)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=17)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=18)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=19)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=20)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
smry <- pf.summary(PF,pf_tax,factor=21)
smry$group1$IDs$otuIDs %>% as.list %>% sapply(.,toString)
...
plot_bar(plot_subset, fill="Phylum",facet_grid="prev_type")
plot_bar(plot_subset, fill="Phylum",facet_grid="type")
...
pfdata <- phyloseq(vst_percent, meta_table, ep_tax, ep_tree_rooted)
plot_subset <- subset_samples(pfdata, prev_type!="NA")
plot_bar(plot_subset, fill="Phylum",facet_grid="type")
